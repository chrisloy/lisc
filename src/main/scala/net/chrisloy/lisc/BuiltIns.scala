...
object BuiltIns {
  ...
  val all: Map[Symbol, Eval] = Map(
    ...
    Symbol("*")  -> { implicit scope => _multiply _ },
    Symbol("/")  -> { implicit scope => _divide _ },
  ...
  def _multiply(args: Args)(implicit scope: Scope): Value = args match {
    case a if a.forall(scope.isLong) => a.map(scope.toLong).reduce(_ * _)
    case a => a.map(scope.toDouble) reduce (_ * _)
  }

  def _divide(args: Args)(implicit scope: Scope): Value = args match {
    case a if a.forall(scope.isLong) => a.map(scope.toLong).reduce(_ / _)
    case a => a.map(scope.toDouble) reduce (_ / _)
  }
